...
def statsTracker(current_lineup, player_track, player, stat):
    ...
    return player_track
h_values = [{'fga':0,'fta':0,'tov':0,'tmfga':0,'tmfta':0,'tmtov':0} for i in range(len(home_players))] #create list of lists for dictionary values for both home and away teams
...
for q in quarters: #looping through each quarter
    ...
    for i in quarter_plays.index:
        if (home_desc[i] == None) and (away_desc[i] == None): pass #nothing happened
            ...
            if ('SUB: ' in home_event): #if home team has substitution
                home_lineup = updateLineup(updateLineup(home_lineup, p2[i]), p1[i], leave = True)
            if ('SUB: ' in away_event): #if away team has substition
                away_lineup = updateLineup(updateLineup(away_lineup, p2[i]), p1[i], leave = True)
                    
            if (home_desc[i] != None): #tracking home team stats
                if ('Free Throw' not in home_desc[i]) and ((' PTS' in home_desc[i]) or ('MISS' in home_desc[i])): #if there's a shot attempt
                    home_track = statsTracker(home_lineup, home_track, p1[i], stat = 'fga') 
                elif 'Free Throw' in home_desc[i]: #if there's a free throw attempt
                    home_track = statsTracker(home_lineup, home_track, p1[i], stat = 'fta')
                elif ('Turnover' in home_desc[i]): #if there's a turnover
                    home_track = statsTracker(home_lineup, home_track, p1[i], stat = 'tov')
            if away_desc[i] != None: #tracking away team stats
                if ('Free Throw' not in away_desc[i]) and ((' PTS' in away_desc[i]) or ('MISS' in away_desc[i])): #if there's a shot attempt
                    away_track = statsTracker(away_lineup, away_track, p1[i], stat = 'fga')
                elif 'Free Throw' in away_desc[i]: #if there's a free throw attempt
                    away_track = statsTracker(away_lineup, away_track, p1[i], stat = 'fta')
                elif ('Turnover' in away_desc[i]): #if there's a turnover
...
def calculateUsageRate(track):
    for player in track:
        ...
        if den == 0:
            ...
            p_dict['usg'] = str(round((num/den)*100,1))+'%'
